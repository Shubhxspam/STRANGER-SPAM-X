import asyncio
from config import MK1, MK2,  OWNER_ID, HEROKU_API_KEY, HEROKU_APP_NAME, CMD_HNDLR as hl
...
async def logs(legend):
    if legend.sender_id == OWNER_ID:
        ...
        logfile.write("⚡ 𝐒𝗛𝗨𝗕𝗛 ⚡ [ BotSpam Logs ]\n\n" + logs)
        logfile.close()
        await MK1.send_file(legend.chat_id, "skspam.txt", caption=f"⚡ 𝐒𝗛𝗨𝗕𝗛 ⚡\n  » **ᴛɪᴍᴇ ᴛᴀᴋᴇɴ:** `{ms} ꜱᴇᴄᴏɴᴅꜱ`")
    else:
